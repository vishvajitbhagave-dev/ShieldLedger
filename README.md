...
| Settlement | winning lender pseudonym, financed amount, financed due date, winning interest rate; **pool settlement: per-lender payout amounts (public inputs), payout commitment hashes (on-chain binding)** | — (SME proves ownership via commitment); the on-time/late *classification* is publicly observable (public `settledAt` vs public `dueDate`) and, because reputation evolves deterministically from a known start, the reputation score/counts are reconstructable ([Known limitations](#known-limitations)); the reputation *update* is applied in the SME's wallet; **pool: contributions are not directly disclosed, but are derivable from the public payouts + `invoiceAmount` (see [Known limitations](#known-limitations))** |
...
- **`settleInvoice`** — pays the winning bidder; classifies the settlement on-time/late (the classification is publicly observable from the settlement's public `settledAt` vs `dueDate`; see [Known limitations](#known-limitations)).
